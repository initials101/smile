...
const appointmentSchema = new mongoose.Schema(
  {
    appointmentId: {
      ...
      default: () => `APT${Date.now()}${Math.random().toString(36).substr(2, 3).toUpperCase()}`,
    },
    patient: {
...
// Pre-save middleware to generate appointment ID if not already set
appointmentSchema.pre("save", async function (next) {
  if (!this.appointmentId || this.appointmentId.includes("APT" + Date.now())) {
    try {
      ...
      this.appointmentId = `APT${Date.now()}${Math.random().toString(36).substr(2, 3).toUpperCase()}`
    }
  }
  next()
})
