...
const patientSchema = new mongoose.Schema(
  {
    ...
    patientId: {
      ...
      default: () => `PAT${Date.now()}${Math.random().toString(36).substr(2, 3).toUpperCase()}`,
    },
    dateOfBirth: {
      type: Date,
...
// Pre-save middleware to generate patient ID if not already set
patientSchema.pre("save", async function (next) {
  if (!this.patientId || this.patientId.includes("PAT" + Date.now())) {
    try {
      const count = await mongoose.models.Patient.countDocuments()
      this.patientId = `PAT${String(count + 1).padStart(6, "0")}`
    } catch (error) {
      // Fallback to timestamp-based ID if count fails
      this.patientId = `PAT${Date.now()}${Math.random().toString(36).substr(2, 3).toUpperCase()}`
    }
  }
  next()
})
